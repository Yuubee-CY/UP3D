<template>
  <div class="Company_News_Body1" ref="Company_News_Body1">
    <div class="text">
...
export default {
  ...
  data() {
    return {
      loadList: this.lang2.news_card.slice(0, 4),
      expFlag: false,
      PHmoreText: '点击查看更多'
    };
  },
  methods: {
    showMore(e) {
      ...
      if(!this.expFlag){
        ...
        this.loadList = this.lang2.news_card
        this.expFlag = true
      }else {
        this.PHmoreText = '点击查看更多'
        this.loadList = this.lang2.news_card.slice(0, 4)
        this.expFlag = false
      }
    },
    showPHMore(e) {
    ...
    golocation(e, index) {
      if (e.target.target.toString() == '_blank') {
        window.open(this.lang2.news_card[index].url, "_blank");
      } else {
        if (this.lang2.news_card[index].url) {
          this.$router.push({path: this.lang2.news_card[index].url})
        }
      }
    }
  },
...
.Company_News_Body1.cn-active {
  overflow: visible;
}

.Company_News_Body1 .text {
  display: flex;
...
.Company_News_Body1 .text .title {
  float: left;
  width: max-content;
  color: #343434;
}

.Company_News_Body1 .text .more {
  float: right;
  line-height: normal;
  color: #707070;
  cursor: pointer;
...
.Company_News_Body1 .text .more i {
...
.Company_News_Body1 .text .more.more-active i {
  transform: rotate(0deg);
}

.Company_News_Body1 .Bigcard {
  max-width: 8.25rem;
  height: max-content;
  margin: auto;
  margin-top: .1875rem;
...
.Company_News_Body1 .Bigcard .card {
  display: inline-block;
  width: 4.015625rem;
  height: auto;
...
.Company_News_Body1 .Bigcard .card .img {
  overflow: hidden;
  height: 1.8125rem;
}

.Company_News_Body1 .Bigcard .card .img img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.Company_News_Body1 .Bigcard .card .t-content {
  width: 3.645833rem;
  margin: auto;
  margin-top: .234375rem;
}

.Company_News_Body1 .Bigcard .card .t-content .time {
  width: 100%;
  font-size: 16PX;
  padding-top: 0rem;
  overflow: hidden;
...
.Company_News_Body1 .Bigcard .card .t-content .name {
  display: block;
  width: 100%;
  font-size: 19PX;
  font-weight: normal;
...
.Company_News_Body1 .Bigcard .card .t-content .name:hover{
  color: #FEBB6F;
}
